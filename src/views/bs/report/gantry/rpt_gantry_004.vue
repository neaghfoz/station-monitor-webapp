    ...
    <el-form ref="searchForm" :model="queryParams" inline :label-width="'100px'" v-bind:style="{height: searchFormHeight}">
      <el-form-item ref="elRowAll" style="margin-bottom:0">
        <el-row>
          ...
          <el-form-item label="门架方向" label-width="80px" prop="ganTryOrder">
            <ti-select style="width:180px" v-model="queryParams.ganTryOrder" dict-type="tibms_bs_report_ganTryOrder"/>
          </el-form-item>
        </el-row>
        <el-row>
          ...
          <el-form-item label="通行交易日期" prop="tansDates" label-width="120px">
            <el-col :span="25">
...
export default {
  name: 'bs.report.gantry.rpt_gantry_004',
  components: {TiSelect, TiSelectTree, TiDateRange, tiSysOrg, TiDateRangeDivide, TiSysRoad, TiSysGantry},
  mixins: [...mixin],
  data() {
    return {
      reportUrl: '',
      urlprefix: '/webroot/decision/view/report?viewlet=gantry/rpt_gantry_004_01_门架通行介质车流量报表(拆分前).cpt',
      queryParams: {
        reportCode: 'gantry_004',
  ...
  methods: {
    ...
    getData() {
      ...
      this.$refs.searchForm.validate((valid) => {
        if (valid) {
          console.log("this.queryParams.roadNo:", this.queryParams.roadNo);
