    ...
    <el-form ref="searchForm" :model="queryParams" inline :label-width="'100px'"
      ...
      <el-form-item ref="elRowAll" style="margin-bottom:0">
        ...
        <el-form-item label="纸张大小" prop="paperType">
          <ti-select style="width:180px" v-model="queryParams.paperType" dict-type="tibms_bs_report_paperType"/>
        </el-form-item>
...
<script>
  ...
  export default {
    name: 'bs.report.stationFlow.rpt_stationFlow_010',
    components: {TiSelect, TiDateRange, tiSysOrg, TiDateRangeDivide},
    mixins: [...mixin],
    data() {
      return {
        reportUrl: '',
        urlprefix: '/webroot/decision/view/report?viewlet=stationFlow/rpt_stationFlow_010_01_路段计费方式分车型统计综合报表_A3.cpt',
        urlprefix1: '/webroot/decision/view/report?viewlet=stationFlow/rpt_stationFlow_010_01_路段计费方式分车型统计综合报表_A3.cpt',
        urlprefix2: '/webroot/decision/view/report?viewlet=stationFlow/rpt_stationFlow_010_01_路段计费方式分车型统计综合报表_A4.cpt',
        queryParams: {
          reportCode: 'stationFlow_010',
    ...
    methods: {
      getData() {
        ...
        if (this.queryParams.dateStatisticType === 'day') {
          ...
          filename = "路段计费方式分车型统计综合年报表(拆分前)";
        }
        if(this.queryParams.paperType == '2'){
          this.urlprefix = this.urlprefix2;
        }else{
          this.urlprefix=this.urlprefix1;
        }
