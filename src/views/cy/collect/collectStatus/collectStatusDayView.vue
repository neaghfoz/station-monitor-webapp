<template>
  <div class="app-container">
    ...
    >
      ...
      <template v-slot:operation="{ row }" width="200px">
        <el-button v-permission="['collectStatus:detail']" type="primary" size="small" v-show="hasPermission(row,'detail')" @click="detail(row)">查看</el-button>
        <el-button v-permission="['collectStatus:add']" type="primary" size="small" v-show="hasPermission(row,'add')" @click="add(row)">去填报</el-button>
        <el-button v-permission="['collectStatus:edit']" type="primary" size="small" v-show="hasPermission(row,'edit')" @click="edit(row)">编辑</el-button>
        <el-button v-permission="['collectStatus:edit2']" type="primary" size="small" v-show="hasPermission(row,'edit2')" @click="edit(row)">编辑</el-button>
        <el-button v-permission="['collectStatus:apply']" type="primary" size="small" v-show="hasPermission(row,'apply')" @click="apply(row)">申请修改</el-button>
        <el-button v-permission="['collectStatus:auth']" type="primary" size="small" v-show="hasPermission(row,'auth')" @click="showAuth(row)">授权修改</el-button>
        <el-button v-permission="['collectStatus:commit']" type="primary" size="small" v-show="hasPermission(row,'commit')" @click="commit(row)">提交</el-button>
...
<script>
  ...
  export default {
    name: 'cy.collect.collectStatus.day',
    mixins: [...mixin],
    ...
    data() {
      return {
        queryParams: {
          ...
          collectType: CollectConstant.type.day
        },
        roadId:'',
        ...
        columns: [
          ...
          {title: '填报日期', field: 'collectDate', minWidth: 100, showOverflow: true},
          ...
          {title: '操作', minWidth: 250, maxWith: 400, fixed: 'right', align: 'center', slots: {default: 'operation'}}
        ],
        applyForm:{
          roadId:'',
          roadName:'',
    ...
    methods: {
      hasPermission(row, opType) {
        ...
        switch (opType) {
          ...
          case 'edit':
            ...
            break;
          case 'edit2':
            flag = userOrgLevel != province && (rowStatus == status.submit);
            break;
          case 'apply':
            break;
          case 'auth':
      ...
      async getCurUserOrg() {
        ...
        this.roadId = (data.nodeLevel == this.constant.sysOrg.NodeLevel.province ) ? '':data.roadId;
        this.statusDictType = (data.nodeLevel == this.constant.sysOrg.NodeLevel.province) ? 'tibms_cy_collect_provice_status':'tibms_cy_collect_road_status';
