  ...
  <div>
    ...
    <div v-else>
      <template>
        <slot :slotProps="slotProps">
          <el-input
            ...
            @focus="ivisible = true"
          >
            <el-button slot="append" icon="el-icon-search" @click="ivisible = true" />
          </el-input>
        </slot>
      </template>
      ...
      >
        ...
        >
          <template v-slot:fileName="{ row }">
            <template v-if="isPicture(row.url)">
              <img :src="getImgUrl(row.url)" width="160" height="90">
            </template>
            <template v-else>
              <a href="javascript:;" @click="clickDownLoad(row)">{{ row.url }}</a>
            </template>
            <!--          <el-progress v-if="uploadPercentage" :percentage="uploadPercentage" status="success" />-->
          </template>
          ...
          <template v-slot:operation="{ row, rowIndex }">
            <div class="aclass">
              <el-button v-if="isPicture(row.url)" type="text" @click="preview(row)">预览</el-button>
      ...
      <el-dialog :visible.sync="previewVisible" title="预览">
        <img :src="previewPic && previewPic.url ? getImgUrl(previewPic.url) : ''" style="width: 100%">
      </el-dialog>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      dataList: [],
      columns: [
        { title: '文件', minWith: 240, slots: { default: 'fileName' }},
    ...
    uploadFile(file) { // todo
      ...
      if (this.data && Object.keys(this.data).length > 0) {
        Object.keys(this.data).forEach(key => {
          FormDatas.append(key, this.data[key])
        })
      }
      this.$http({
        url: 'api/v1/system/uploadFile',
