        ...
        return {
          disabledDate: time => {
            ...
            if (!!self.choiceDate) {
              ...
              const isDayVal = new Date(new Date(self.choiceDate).getFullYear(),new Date(self.choiceDate).getMonth() + 1,0).getDate() - new Date(self.choiceDate).getDate();
              let maxDayVal = isDayVal;
              if(maxDayVal >= 15) {
                maxDayVal = 14;
              }
              const endDay = maxDayVal * 24 * 3600 * 1000;
              let minTime = self.choiceDate;
              ...
              switch (this.dateRule) {
                ...
                case 'oneYear':
                  flag = curDateStr < moment(this.selectDate).add(-12, 'months').format(
                    "YYYY-DD") || curDateStr > moment(this.selectDate).add(12, 'months').format("YYYY-DD")
                  break;
              }
            }
            return flag;
