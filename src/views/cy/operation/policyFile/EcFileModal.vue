...
export default {
  ...
  methods: {
    ...
    onError(e, file, filelist) {
      // file._id = guid()
      this.$notify.error('附件超过30M请重新选择！')
      this.$refs.fileUpload.uploadFiles = this.$refs.fileUpload.uploadFiles.filter(item =>item.name!==file.name)
      filelist =  filelist.filter(item =>item.name!==file.name)
      this.$emit('onError',e, file, filelist)
    },
