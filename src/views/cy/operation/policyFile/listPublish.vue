      ...
      <div class="box">
        ...
        >
          <template v-slot:operation="{ row }">
            ...
            <el-button v-permission="['policyFileMain:withdraw']" v-show="row.allowStop" size="mini" type="text" @click="withdraw(row)">撤回</el-button>
          </template>
...
import { list, withdraw, getUserNodeLevel } from './policyFile'
import TiExportExcel from '@/views/pro/common/tiElement/tiExport/tiExportExcel'
import TiDateRange from '@/views/pro/common/tiElement/tiDate/tiDateRange'
import moment from 'moment'
export default {
  name: 'OperationPolicyFileListPublish',
    ...
    return {
      ...
      tableData: {
        ...
        tableColumns: [
          ...
          { title: '标题', field: 'title', minWidth: 150, showOverflow: true, align: 'center' },
  ...
  methods: {
    ...
    async getData() {
      ...
      list(Object.assign({}, { size: this.tableData.page.pageSize, current: this.tableData.page.currentPage,type:'publish' }), this.queryParams).then(res => {
    ...
    view(row) {
      this.$router.push({
        path: '/operations/policyFile/publish/details',
        query: { id: row.id }
      })
    },
    withdraw(row) {
