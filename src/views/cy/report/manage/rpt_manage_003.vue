...
<script>
  ...
  export default {
    name: 'report.manage.rpt_manage_003',
    ...
    methods: {
      async getData() {
        ...
        if (this.queryParams.dateStatisticType === 'month') {
          ...
          this.queryParams.fileName = sysOrg.name + '-折算全程车流收入汇总月报表-' + moment(new Date()).format('YYYY-MM-DD');
        }
          ...
          dateTrans = dateUtil.collectTypeCombination(this.queryParams.startDay,
            ...
            this.queryParams.dateStatisticType);
          this.queryParams.fileName = sysOrg.name + '-折算全程车流收入汇总日报表-' + moment(new Date()).format('YYYY-MM-DD');
        }
        ...
        this.reportUrl = fineReportUtil.getUrl(
          '/ReportServer?reportlet=report/manage/rpt_manage_003_01_折算全程车流收入汇总.cpt&op=write',
          this.queryParams);

        //更新查询统计次数
        //updateRptSearchTimes('rpt_manage_003');
