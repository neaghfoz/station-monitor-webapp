...
<script>
  ...
  export default {
    name: 'report.manage.rpt_manage_004',
    ...
    methods: {
      async getData() {
        ...
        dateTrans = dateUtil.collectTypeCombination(this.queryParams.startMonth,
          this.queryParams.endMonth, 'naturalDate','month');
        ...
        //this.queryParams.maxYear = this.queryParams.collectDateStr.slice(0,4);
         this.queryParams.collectDateStr = this.queryParams.startMonth + '至' + this.queryParams.endMonth;
        ...
        this.queryParams.fileName = sysOrg.name + '-断面车流量收入明细表月报-' + moment(new Date()).format('YYYY-MM-DD');
        //用户levelya
        ...
        this.reportUrl = fineReportUtil.getUrl(
          '/ReportServer?reportlet=report/manage/rpt_manage_004_01_断面车流量收入明细表.cpt',
          this.queryParams);
