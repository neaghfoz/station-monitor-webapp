...
<script>
  ...
  export default {
    name: 'report.manage.rpt_manage_005',
    ...
    methods: {
      async getData() {
        ...
        this.queryParams.fileName = sysOrg.name + '-收费站车流收入月报-' + moment(new Date()).format('YYYY-MM-DD');
        ...
        this.reportUrl = fineReportUtil.getUrl(
          '/ReportServer?reportlet=report/manage/rpt_manage_005_01_收费站车流收入月报.cpt&op=write',
          this.queryParams);
