    ...
    methods: {
      async getData() {
        ...
        if (this.queryParams.dateStatisticType === 'month') {
          this.queryParams.fileName = sysOrg.name + '-按车型收费站免费车流与收入损失月报-' + moment(new Date()).format('YYYY-MM-DD');
        }else if(this.queryParams.dateStatisticType === 'day')
        {
          this.queryParams.fileName = sysOrg.name + '-按车型收费站免费车流与收入损失日报-' + moment(new Date()).format('YYYY-MM-DD');
        }
        this.reportUrl = fineReportUtil.getUrl(
          '/ReportServer?reportlet=report/manage/rpt_manage_008_按车型收费站免费车流与收入损失月报.cpt',
          this.queryParams);

        //更新查询统计次数
        // updateRptSearchTimes('rpt_manage_008');
      }
    }
  }
</script>
