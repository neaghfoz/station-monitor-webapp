...
<script>
  ...
  import {getMonthFirstDate} from "@/views/cy/common/datePrecision/datePrecision";
  import {getMonthLastDate} from "@/views/cy/common/datePrecision/datePrecision";
  //import {supplementMonth} from "@/views/cy/common/datePrecision/datePrecision";
  import reportUtil from "@/views/pro/report/common/reportUtil"

  export default {
    name: 'report.station.rpt_station_001',
    ...
    methods: {
      async getData() {
        ...
        //参数
        this.queryParams.fileName = sysOrg.name + '月收费情况统计表';
        ...
        this.queryParams.startMonth = getMonthFirstDate(this.queryParams.monthValue);
        this.queryParams.endMonth = getMonthLastDate(this.queryParams.monthValue);

        /*var monthValueArray = this.queryParams.monthValue.split("-");
        var yearVal = monthValueArray[0];
        var monthVal = supplementMonth(monthValueArray[1]-1);
        var lastMonthValue = yearVal + "-" + monthVal;
        this.queryParams.lastStartMonth = getMonthFirstDay(lastMonthValue);
        this.queryParams.lastEndMonth = getMonthLastDay(lastMonthValue);*/
        console.log("queryParams:", this.queryParams);
        this.reportUrl = fineReportUtil.getUrl('/ReportServer?reportlet=report/station/rpt_station_001_01_收费情况统计表.cpt',this.queryParams);
      }
    }
  }
</script>
