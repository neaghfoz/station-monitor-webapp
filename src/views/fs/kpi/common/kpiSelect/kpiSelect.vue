...
import {getData} from './kpiSelectApi'
...
export default {
  ...
  props: {
    ...
    filterEnable: { //是否过滤掉禁用的字典项,默认不过滤
      type: Boolean,
      default: false,
      required: false
    },
  ...
  methods: {
      ...
      if (!this.dictType) {
        ...
        if(this.filterEnable){
          this.selectData.forEach(x => {
            if(x.valid == '1'){//需要后台增加是否启用的这个标记字段@20210722
              this.selectDataShow.push(x)
            }
          })
        }else{
    ...
    handleNativeBlur(e){
      // 上层实现此方法，实现鼠标移开时能保证输入搜索值未匹配也不被清除@20210722
      this.$emit('blur', e)
    }
  }
}
