    ...
    >
      <el-form ref="form" v-loading="loading" :model="formItem" :rules="rules">
        <el-form-item
          ...
          label="收费员"
        >
          <ti-select v-model="formItem.dutyOperatorNo" url="/api/v1/common/sysUser/list" :props="{key:'id',value:'username',label:'fullName'}"/>
        </el-form-item>
        <vxe-grid
          ref="table"
          border
...
import { use, checkPaper } from '../paperApi'
import TiSelect from '@/views/pro/common/tiElement/tiSelect/tiSelect'

export default {
  name: 'paperoiceUseForm',
  components: {
    TiSelect
  },
  mixins: [...mixin],
  data() {
    return {
      ...
      table: {
        ...
        columns: [
          { field: 'paperStartShowNo', title: '库存纸券起始号', minWidth: 130, align: 'center',
            editRender: { }, slots: { edit: 'paperStartNoShowEdit' }},
          { field: 'paperEndShowNo', title: '库存纸券终止号', minWidth: 130, align: 'center',
            editRender: { }, slots: { edit: 'paperEndNoShowEdit' }},
          { field: 'paperStartNo', title: '领用纸券起始号', minWidth: 150, align: 'center',
            editRender: { }, slots: { edit: 'paperStartNoEdit' }},
          { field: 'paperEndNo', title: '领用纸券终止号', minWidth: 150, align: 'center',
  ...
  methods: {
    ...
    render: function(tenantId, appId, rows) {
      ...
      console.log(this.formItem.results)
    },
    removeRecord(row) {
      const that = this
      ...
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          if (this.formItem.results.length === 0) {
            this.$notify.error('至少领用一条纸券号段记录')
            return
          }
