...
import { returns, checkExistsInv } from '../rinvoiceApi'

export default {
  name: 'InvoiceReturnForm',
  components: {
  },
  mixins: [...mixin],
  data() {
    return {
      ...
      table: {
        ...
        columns: [
          ...
          { field: 'invStartShowNo', title: '领用发票起始号', minWidth: 130, align: 'center',
            editRender: { }, slots: { edit: 'invStartNoShowEdit' }},
          { field: 'invEndShowNo', title: '领用发票终止号', minWidth: 130, align: 'center',
            editRender: { }, slots: { edit: 'invEndNoShowEdit' }},
          { field: 'invStartNo', title: '归还发票起始号', minWidth: 130, align: 'center',
            editRender: { }, slots: { edit: 'invStartNoEdit' }},
          { field: 'invEndNo', title: '归还发票终止号', minWidth: 130, align: 'center',
      ...
      this.$refs.form.validate(async valid => {
        if (valid) {
          if (this.formItem.results.length === 0) {
            this.$notify.error('至少归还一条发票号段记录')
            return
          }
