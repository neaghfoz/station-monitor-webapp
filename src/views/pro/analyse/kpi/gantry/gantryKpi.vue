<template>
  <div class="app-container">
    <el-tabs v-model="queryParams.activeName" type="card"  @tab-click="handleClick">
      ...
      <el-tab-pane label="收费门架" name="gantry"></el-tab-pane>
      <el-tab-pane label="时间" name="time"></el-tab-pane>
    </el-tabs>
    <div class="search-form">
      <el-form ref="searchForm" :model="queryParams" inline :label-width="'100px'">
        <el-form-item label="机构" prop="sysOrgIdStr" v-show="gantryFlag || timeFlag" >
          <ti-sys-org ref="sysOrg" v-model="queryParams.sysOrgIdStr"  default-first-value show-level="2" />
        </el-form-item>
        <el-form-item label="门架名称" v-show="timeFlag" prop="gantryIdStr">
          <ti-select
            ref="gantry"
        ...
        <el-form-item label="统计类型" prop="kpiType" v-show="kpiTypeFlag"   >
          <ti-select v-model="queryParams.kpiType"  dict-type="tibms_analyse_gantryTrans_kpiType" placeholder="全部"   :clearable="false"   />
        </el-form-item>
...
<script>
  ...
  import tableOption from "@/views/pro/analyse/kpi/gantry/tableOption"
  import {getData} from "./gantryKpiApi"
  import moment from "moment"
  import chartOption from "@/views/pro/analyse/kpi/gantry/chartOption"
  import tiSysOrg from "@/views/pro/common/tiElement/tiSysOrg/tiSysOrg";
  import dictUtils from 'ecip-web/utils/dictUtils';
  import {getOrgTreeData} from "@/views/pro/common/util/dataUtil";

  export default {
    name: "analyse.kpi.gantry.gantryKpiView.vue",
    mixins: [...mixin],
    ...
    methods: {
      ...
      async reload(isCache){
        ...
        if(!isCache){
          ...
          // param.sysOrgIdStr = param.sysOrgIdStr.id;
          if(this.queryParams.activeName === 'road' ){
            param.sysOrgIdStr='';
            param.gantryIdStr='';
          } else if(this.queryParams.activeName === 'gantry'){
            param.gantryIdStr='';
          }
        ...
        if(this.queryParams.showDefault=='table'){
          ...
          let kpiTypeList = dictUtils.getDictList("tibms_analyse_gantryTrans_kpiType");
          //图表显示seriname
          let kpiTypeName ;
