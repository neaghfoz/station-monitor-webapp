  ...
  <div class="app-container" style="height: 100%" v-loading="exportExcel.loading">
    ...
    >
      ...
      <template v-slot:operation="{ row }">
        <el-button type="warning" @click="analyse(row)">校核分析</el-button>
      </template>
    </vxe-grid>
  </div>
</template>
...
import { pageByDate } from "@/views/pro/audit/auditSplitDiffSumRpt/api/auditSplitDiffSumRptApi"
...
export default {
  name: "audit.auditSplitDiffSumRpt.auditSplitDiffSumRptView",
  ...
  data () {
    return {
      ...
      columns: [
        ...
        {
          field: '#', title: '拆分实收', align: 'center', children: [
            { field: 'splitTransNumStr', title: '行程数', align: 'center', minWidth: 100, showOverflow: true, },
            {
              field: 'splitTransFee4YuanStr', title: '拆分实收(元)', align: 'center', minWidth: 100, showOverflow: true
            }
          ]
        },
        {
  ...
  methods: {
    ...
    exportEx () {
      ...
      download({
        url: "api/v1/audit/feeAnalyse/auditSplitDiffSumRpt/exportAuditSplitDiffSumRptByDate",
        method: 'post',
        params: null,
        data: params
      }).then((res) => {
