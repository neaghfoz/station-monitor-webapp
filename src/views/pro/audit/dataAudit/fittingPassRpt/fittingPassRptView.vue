  ...
  <div class="app-container" style="min-height: calc(100vh - 84px);display: flex;flex-direction: column;">
    <el-form ref="appForm" :model="queryParams" inline
      ...
      <el-form-item class="searchItem">
        <el-button type="primary" v-permission="['fittingPassRpt:list']" @click="submit">查询</el-button>
    ...
    <div style="flex:1;min-height: 400px;">
      <vxe-grid
        ...
        <template v-slot:tbars>
          <el-button v-permission="['fittingPassRpt:exp']"type="primary" @click="expWork">导出</el-button>
        </template>
...
<script>
  ...
  import api from "@/views/pro/audit/dataAudit/fittingPassRpt/fittingPassRptApi"
  ...
  export default {
    name: "pro.audit.dataAudit.fittingPassRpt.fittingPassRptView",
    ...
    data() {
      return {
        ...
        table:{
          ...
          tableToolbar: {
            id: 'audit.fittingPassRptView.toolbar',
            custom: true,
            slots: {
              buttons: 'tbars'
            },
          ...
          columns: [
            {title: '通行交易日期', field: 'lastTransDay', minWidth: 80, align: 'center',showOverflow: true},
            {title: '拟合路径总量', field: 'fittingCount', minWidth: 80, align: 'center',showOverflow: true},
            {title: '自动稽核路径', field: 'autoAuditCount', minWidth: 80, align: 'center',showOverflow: true},
            {title: '异常路径', field: '#',  align: 'center',
              children: [
                {field: 'exceptionCount1', title: '二义性路径', align: 'center', minWidth: 80, showOverflow: true},
                {field: 'exceptionCount2', title: '不可达路径', align: 'center', minWidth: 80, showOverflow: true},
                {field: 'exceptionCount3', title: '非路界出口路径', align: 'center', minWidth: 80, showOverflow: true},
                {field: 'exceptionCount4', title: '非正常出口路径', align: 'center', minWidth: 80, showOverflow: true},
                {field: 'exceptionCount5', title: '孤立点', align: 'center', minWidth: 80, showOverflow: true}
              ]
            },
            {title: '异常路径占比', field: 'exceptionCountRate', minWidth: 80, align: 'center',showOverflow: true},
            {title: '已稽核路径数量', field: 'manualAuditCount', minWidth: 80, align: 'center', showOverflow: true}
          ]
        },
        exportExcel: {
    ...
    methods: {
      ...
      async reload(){
        if (this.queryParams.beginDate == null) {
          ...
          this.queryParams.beginDateStr = this.queryParams.beginDate.replace(/-/g,"")
        }
        if (this.queryParams.endDate == null) {
          this.queryParams.endDateStr = null
        } else{
          this.queryParams.endDateStr = this.queryParams.endDate.replace(/-/g,"")
        }
