  ...
  <div class="app-container">
    ...
    <vxe-grid
      ...
      <template v-slot:bottom>
        1、自然日合计数=ETC成功处理量+ETC失败处理量+CPC发卡量+纸券发卡量，站省入口站自然日合计数接口中无统计无通行介质数量；<br>
        2、差异数量=自然日合计数-流水采集数（不含无通行介质）；<br>
        3、统计均不包含冲减流水；
      </template>
<!--      <template v-slot:bottom>-->
<!--        2、差异数量=自然日合计数-流水采集数（不含无通行介质）；-->
<!--      </template>-->
...
<script>
  ...
  import {findEnCountPage} from "@/views/pro/audit/recvCheckStat/components/recvCheckStatDetail";
  ...
  export default {
    name: 'audit.recvCheckStat.components.recvCheckStatEnCountDetail',
    mixins: [...mixin],
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            slots: {
              default: ({row}, h) => {
                if (row.diffCnt != null && (row.diffCnt > this.recvCheckEnMaxNum || row.diffCnt< this.recvCheckEnMaxNum*-1)) {
                  return [h('span', {
                    style: {
                      color: 'white',
                      background: 'red',
    ...
    mounted() {
      let recvCheckEnMaxNum = dictUtils.getDictLabel("tibms_config", "recvCheckEnMaxNum");
      console.log (recvCheckEnMaxNum)
      if( recvCheckEnMaxNum ) {
        this.recvCheckEnMaxNum = recvCheckEnMaxNum;
      }
    ...
    methods: {
      parseParams (params) {
        ...
        this.title = params.roadName + '的' + params.beginDate +'日入口站处理合计数'
        //console.log("queryParams:", this.queryParams);
      },
      async loadList () {
        this.loading = true;
        const { data } = await findEnCountPage(Object.assign({}, {size: this.page.pageSize, current: this.page.currentPage}), this.queryParams);
      ...
      async getData() {
         ...
         const {data} = await findEnCountPage(
