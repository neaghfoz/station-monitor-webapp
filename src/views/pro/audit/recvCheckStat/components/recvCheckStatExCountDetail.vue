  ...
  <div class="app-container">
    ...
    <vxe-grid
      ...
      <template v-slot:bottom>
        1、自然日合计数=ETC成功处理量+ETC失败处理量+CPC成功交易量+CPC失败交易量+纸券成功交易量+纸券失败交易量；站省出口站自然日合计数接口中无统计无通行介质数量；<br>
        2、自然日合计金额=CPC成功交易额+CPC失败交易额+纸券成功交易额+纸卷失败交易额；站省出口站处理合计数接口中无ETC成功交易额、ETC失败交易额、无通行介质交易额；<br>
        3、差异数量=自然日合计数-流水采集数（不含无通行介质）；差异金额=自然日合计金额-流水汇总金额（不含ETC交易、无通行介质交易金额）；<br>
        4、统计均不包含冲减流水。
      </template>
...
<script>
  ...
  import {findExCountPage} from "@/views/pro/audit/recvCheckStat/components/recvCheckStatDetail";
  ...
  export default {
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            title: "流水汇总金额(不含ETC交易，无通行介质交易金额)",
            minWidth: 180,
            showOverflow: true,
            align: "center"
          },
          ...
          },
          {
            ...
            slots: {
              default: ({row}, h) => {
                if (row.diffCnt != null && (row.diffCnt > this.recvCheckExMaxNum  || row.diffCnt< this.recvCheckExMaxNum*-1)) {
                  return [h('span', {
                    style: {
                      color: 'white',
                      background: 'red',
            ...
            slots: {
              default: ({row}, h) => {
                if (row.diffFeeSumText != null && row.diffFeeSumText != 0 && row.diffCnt != null
                  && (row.diffCnt > this.recvCheckExMaxNum || row.diffCnt< this.recvCheckExMaxNum*-1)) {
                  return [h('span', {
                    style: {
                      color: 'white',
                      background: 'red',
    ...
    mounted() {
      let recvCheckExMaxNum = dictUtils.getDictLabel("tibms_config", "recvCheckExMaxNum");
      console.log (recvCheckExMaxNum)
      if( recvCheckExMaxNum ) {
        this.recvCheckExMaxNum = recvCheckExMaxNum;
      }
    ...
    methods: {
      parseParams (params) {
        ...
        this.title = params.roadName + '的' + params.beginDate +'日出口站处理合计数'
        //console.log("queryParams:", this.queryParams);
      },
      async loadList () {
        this.loading = true;
        const { data } = await findExCountPage(Object.assign({}, {size: this.page.pageSize, current: this.page.currentPage}), this.queryParams);
      ...
      async getData() {
         ...
         const {data} = await findExCountPage(
