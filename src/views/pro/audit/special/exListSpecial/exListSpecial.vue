<template>
  <div class="app-container">
    <el-form ref="searchForm" :rules="searchRules" :model="queryParams" inline
             :label-width="'100px'">
      <el-row>
        ...
        <el-form-item label="出口车道号" prop="laneNo">
          <el-input v-model="queryParams.laneNo" type="text" placeholder="出口车道号" maxlength="5"
        ...
        <el-form-item label="交易支付方式" prop="transPayTypes">
          <ti-select v-model="queryParams.transPayTypes" dict-type="tibms_com_transPayType"
                     :multiple="true"/>
        </el-form-item>
      </el-row>
      <el-row>
      ...
      <el-row>
        <el-form-item label="入口时间">
          <el-form-item prop="enBeginDate" v-show="false"/>
          <el-form-item prop="enEndDate" v-show="false"/>
          <ti-date-range ref="enDateRang" date-type="datetimerange"
                         format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"
                         date-rule="singleMonth" v-model="queryParams" begin-key="enBeginDate"
                         end-key="enEndDate"/>
        </el-form-item>
        ...
        <el-form-item label="通行介质编码" prop="mediaNo">
          <el-input v-model="queryParams.mediaNo" type="text"
                    placeholder="通行介质编码" maxlength="20"/>
        </el-form-item>
      </el-row>
      <el-row>
        <el-form-item label="计费方式" prop="exitFeeTypes">
          <ti-select v-model="queryParams.exitFeeTypes" dict-type="tibms_com_exitFeeType"
                     :multiple="true"/>
        </el-form-item>
        <el-form-item label="出口车种" prop="vehicleClasses">
          <ti-select v-model="queryParams.vehicleClasses" dict-type="tibms_com_vehicleClass"
                     :multiple="true" :disabled="allowVehclass"/>
        </el-form-item>
        <el-form-item label="复核来源" label-width="100px" prop="recheck" v-if="searchType == 'exSpecial'">
        ...
        <el-form-item label="本地特情类型" prop="roadSpecialTypeSelect">
          <ti-select
            v-model="queryParams.roadSpecialTypeSelect"
            dict-type="tibms_com_specialType_local"
            :multiple="true"
            :disabled="allowRoadSpecial"/>
        </el-form-item>
      ...
      <el-row>
        <div v-show="showAuditSearch" class="search-form">
          ...
          <el-row>
            <el-form-item label="出口处理编号" prop="serialNo">
              <el-input v-model="queryParams.serialNo" type="text" placeholder="出口处理编号"
                        maxlength="38"/>
            </el-form-item>
            ...
            <el-form-item label="出口收费员" prop="exOperatorId">
              <el-input v-model="queryParams.exOperatorId" type="text" placeholder="出口收费员ID"
                        maxlength="21"/>
            </el-form-item>
          </el-row>
          <el-row>
    ...
    <vxe-grid
      ...
      <template v-slot:toolbar_buttons>
        <el-button v-permission="['exListSpecial:exportExcel']" @click="exportExcel.dialog = true">
          导出
        </el-button>
      </template>
    ...
    <el-drawer
      ...
      direction="rtl"
      size="82%">
      <ti-audit-panel ref="audit" :log-columns="logColumns" @submit="stationAuditSubmit" :is-ex-list="true" style="margin-top: 10px;">
...
<script>
  ...
  import {auditEx, findPageEx, getEnVehicleSigId} from "@/views/pro/audit/special/exListSpecial/exListSpecialApi";
  ...
  export default {
    name: 'audit.special.exListSpecial.exListSpecialView',
    mixins: [...mixin],
    ...
    data() {
      return {
        ...
        queryParams: {
          ...
          mediaTypes: '',
          mediaNo: '',
          recheck: this.searchType == 'exSpecial' ? '1':'',
          specialTypes: '',
          roadSpecialTypes:'',
          specialTypeSelect: '',
          roadSpecialTypeSelect:'',
          sysOrgIdStr: '',
          sysOrg: {},
          auditStatusStation: '',
          auditResultStation: '',
    ...
    created() {
      console.log('this.queryParams.searchType22222',this.searchType)
    },
    watch:
      {
        ...
        'roadAudit.auditFeeRoadStr':
          {
            handler(v) {
              if (v) {
                console.log("auditFeeRoadStr:", v);
                console.log("feeStr:", this.roadAudit.feeStr);
    ...
    mounted() {
      console.log('this.queryParams.searchType',this.searchType)
      if(this.queryParams.searchType == 'cars'){
        this.queryParams.vehicleClasses = '14';
        this.allowVehclass = true;
      }else if(this.queryParams.searchType == 'rmergencyCar'){
        this.queryParams.vehicleClasses = '10';
        this.allowVehclass = true;
      }else if(this.queryParams.searchType == 'reInvoice'){
        this.queryParams.roadSpecialTypeSelect = '2';
        this.allowRoadSpecial = true;
      }else if(this.queryParams.searchType == 'rushWater'){
        // this.queryParams.roadSpecialTypes = [''];
        // this.roadSpecialTypes = true;
      };
    ...
    methods: {
        ...
        if (this.tableData.length > 0) {
          ...
          this.activeName = 'auditDetail',
            this.$nextTick(() => {
              ...
              this.detailData = this.$refs.table.data[0];
      ...
      async getImg(v) {
        let enVehicleSignId = await getEnVehicleSigId(v.passId, v.enTime).then(res => {
        ...
        if (proId == 14) {
          this.exImageUrl = 'data:image/jpg;base64,' + (await getSxImgBase64(
              {roadId: v.exSectionId, stationId: v.exStationId}
              , {picId: v.id, picTime: v.exTime ? v.exTime.replace(' ', 'T') : null, vehiclePlate: null, gantryId: null, imgType: "1"})
          ).data;
          this.enImageUrl = 'data:image/jpg;base64,' + (await getSxImgBase64(
              {roadId: v.enSectionId, stationId: v.enStationId}
              , {picId: enVehicleSignId, picTime: v.enTime ? v.enTime.replace(' ', 'T') : null, vehiclePlate: null, gantryId: null, imgType: "1"})
          ).data;
        } else {
          this.exImageUrl = 'data:image/jpg;base64,' + (await getImgBase64(v.exSectionId, v.exStationId, v.vehicleSignId + '_' + ImgConstant.gdImgFix.frontPic)).data;
          this.enImageUrl = 'data:image/jpg;base64,' + (await getImgBase64(v.enSectionId, v.enStationId, enVehicleSignId + '_' + ImgConstant.gdImgFix.frontPic)).data;
        }

      },
      async getEnImgBase64(id, detail) {
        this.enImageUrl = 'data:image/jpg;base64,' + (await getImgBase64(detail.enSectionId, detail.enStationId, id + '_' + ImgConstant.gdImgFix.frontPic)).data;
      },
      ...
      async audit(params) {
        ...
        this.$nextTick(() => {
          ...
          if (data[0]) {
            ...
            this.$refs.table.setCurrentRow(this.$refs.table.data[this.selectRowIndex]);
            this.detailData = this.$refs.table.data[this.selectRowIndex];
            // this.getImg(this.detailData);
            // this.copyValue(this.detailData);
      ...
      roadAuditSubmit() {
        this.$refs.roadAuditForm.validate((valid) => {
          if (valid) {
            ...
            this.roadAudit.exTime = this.detailData.exTime;
      ...
      watchAttrFn(newVal, oldVal, selectData) {
       ...
       watchAttrFnRoad(newVal, oldVal, selectData) {
        ...
        this.queryParams.roadSpecialTypes = roadSpecialTypes.join(',');
        return selectDataShow;
      },
      ...
      copyValue(value) {
        ...
        if (value.auditStatusRoad && value.auditStatusRoad > 0) {
          ...
          this.roadAudit.auditVehicleTypeRoad = value.exVehicleType.toString();
          this.roadAudit.auditVehicleClassRoad = value.exVehicleClass.toString();
          this.roadAudit.auditAxleCountRoad = value.exAxleCount;
        ...
        if (value.auditStatusStation && value.auditStatusStation > 0) {
          ...
          this.stationAudit.auditVehicleTypeStation = value.exVehicleType.toString();
          this.stationAudit.auditVehicleClassStation = value.exVehicleClass.toString();
          this.stationAudit.auditAxleCountStation = value.exAxleCount;
