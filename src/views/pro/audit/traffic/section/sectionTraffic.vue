    ...
    <el-form :model="appForm" ref="appForm" :rules="rules" :show-message="false" label-width="90px" label-position="right" :inline="true">
      <el-row>
        <el-form-item label="机构" prop="sysOrgIdStr">
          <ti-sys-org ref="sysOrg" v-model="appForm.sysOrgIdStr"
                      default-first-value show-level="2" style="width: 177px"/>
        </el-form-item>
      ...
      <el-row>
        ...
        <el-form-item  v-show="totalDay" >
          <button type="button" class="vxe-button type--button size--mini theme--primary" @click="$data.totalDay='';reload(true);">
            <span class="vxe-button--content">{{ totalDay }}</span>&nbsp;
            <i class="vxe-button--icon vxe-icon--close"></i>
          </button>
        </el-form-item>
      </el-row>
    </el-form>
...
<script>
  ...
  import tableOption from "@/views/pro/audit/traffic/section/tableOption"
  import {reqData} from "@/views/pro/audit/traffic/section/sectionTrafficApi"
  import moment from "moment";
  import chartOption from "@/views/pro/audit/traffic/section/chartOption"
  import tiSysOrg from "@/views/pro/common/tiElement/tiSysOrg/tiSysOrg"


  export default {
    name: 'audit.traffic.section.sectionTrafficView',
    mixins: [...mixin],
    ...
    computed:{
      columns(){
        return this.table.showDefault=='vc' ? tableOption.vcColumns(this.totalDay) : tableOption.vtColumns(this.totalDay)
      }
    },
    mounted(){
      // 初始化图表
    ...
    methods: {
      ...
      reset() {
        ...
        this.appForm.sysOrgIdStr=this.$refs.sysOrg.getDefaultFirstValue();
      },
