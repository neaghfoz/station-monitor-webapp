  ...
  methods: {
    ...
    async getImg(){
      ...
      if(this.isEnList){
        if (proId == 14) {
          this.enImageUrl = 'data:image/jpg;base64,' + (await getSxImgBase64(
              ...
              , {picId: this.dataInfo.id, picTime: this.dataInfo.enTime ? this.dataInfo.enTime.replace(' ', 'T') : null, vehiclePlate: null, gantryId: null, imgType: "1"})
          ).data;
        } else {
      ...
      if(this.isBreakPass && this.dataInfo.exStationId!=null){
        if (proId == 14) {
          this.vehImageUrl = 'data:image/jpg;base64,' + (await getSxImgBase64(
              {roadId: this.dataInfo.exSectionId, stationId: this.dataInfo.exStationId}
              , {picId: this.dataInfo.vehicleSignId, picTime: this.dataInfo.exTime ? this.dataInfo.exTime.replace(' ', 'T') : null, vehiclePlate: null, gantryId: null, imgType: "1"})
          ).data;
        } else {
