...
import api from "@/views/pro/fitcheck/fee/indexFee/indexFeeApi"
...
export default {
  ...
  option: {
    "line": {
      ...
      legend: {
        data: (systemName && systemName === 'tibms_cy') ? ['理论应得', '拟合应得', '拆分实得']: ['理论应收', '拟合应收', '拆分实收'],
        textStyle: {color: "#666"},
        top: '3%',
        right: '3%',
        itemGap: 20
      ...
      series: [{
        name: (systemName && systemName === 'tibms_cy') ? '理论应得':'理论应收',
        type: 'line',
        symbol: 'circle',
        symbolSize: 8,
        itemStyle: {
        ...
        name: (systemName && systemName === 'tibms_cy') ? '拟合应得':'拟合应收',
        type: 'line',
        symbol: 'circle',
        symbolSize: 8,
        itemStyle: {
        ...
        name: (systemName && systemName === 'tibms_cy') ? '拆分实得':'拆分实收',
        type: 'line',
        symbol: 'circle',
        symbolSize: 8,
        itemStyle: {
  ...
  getOption: function (val, $vm, chartId) {
    ...
    switch (type) {
      case "line":
        ...
        if(!(tibms && tibms.fitcheck && tibms.fitcheck.showTheory)){
          if(systemName && systemName === 'tibms_cy'){
            opt.legend.data=['拟合应得', '拆分实得'];
            opt.legend.selected = {'理论应得': false};
          }else{ 
            opt.legend.data=['拟合应收', '拆分实收'];
            opt.legend.selected = {'理论应收': false};
          }
        }
        break;
  ...
  ydtxf(data, $vm) {
    ...
    var provDatas = data.earningsStat;//拆分实收
  ...
  feeSplit(rows, $vm) {
    ...
    if (rows.dataCurrentMonth.length > 0) {
      ...
      if ($vm.$data.top.st1.payFee > 0) {
        $vm.$data.top.st1.diffFee = (parseFloat($vm.$data.top.st1.splitFee - $vm.$data.top.st1.payFee ) / $vm.$data.top.st1.payFee * 100).toFixed(2) + '%'
      }
    }
