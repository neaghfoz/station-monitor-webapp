...
<!--      </el-form-item>-->
      ...
      <el-form-item ref ="elRowAll" style="margin-bottom:0">
        <el-row>
          ...
          <el-form-item label="收费单元类型" label-width="120px" prop="tollInterType">
            <ti-select    v-model="queryParams.tollInterType" dict-type="tibms_com_tollIntervalType"/>
          </el-form-item>
          ...
          <el-form-item label="统计维度" label-width="80px" prop="statLat">
            <ti-select :clearable="false"   v-model="queryParams.statLat" dict-type="tibms_rpt_theory_statLat"/>
          </el-form-item>
        </el-row>
        <el-row>
          <el-form-item label="通行交易日期"  prop="tansDates">
            <el-col :span="30">
              <el-form-item v-show="queryParams.dateStatisticType==='day'">
                ...
                <ti-date-range date-style="width:495px" ref="dayRang" :editable="false" v-model="queryParams" date-type="daterange"
              ...
              <el-form-item v-show="queryParams.dateStatisticType==='month'">
                ...
                <ti-date-range date-style="width:495px" ref="monthRang" :editable="false" v-model="queryParams" date-type="monthrange"
              ...
              <el-form-item v-show="queryParams.dateStatisticType==='year'">
                ...
                <ti-date-range-divide date-style="width:227px" ref="year" :editable="false" v-model="queryParams"
                                      date-type="year"
...
<script>
  ...
  import {updateRptSearchTimes} from "@/views/pro/fitcheck/report/common/rptSearchTimes";
  ...
  export default {
    name: 'fitcheck.report.fitting.cloud.rpt_fitting_001',
    ...
    data() {
      return {
        ...
        queryParams: {
          ...
          tollIntervalIdStr:'',
          startDay:dateUtil.getNextDate(new Date(),'days',6,'YYYY-MM-DD'),
    ...
    computed: {
      rptPrex(){
        return tibms && tibms.fitcheck && tibms.fitcheck.model === 'cloud' ? '/cloud' : '/local'
      },
      propAtr() {
        return {
          sectionId: this.queryParams.sectionId,
          tollInterType: this.queryParams.tollInterType
        }
      }
    },
    created() {
    ...
    methods:{
      ...
      sectionFilter(datas){
        ...
        if(!this.queryParams.sectionId && seArr.length>0){
          ...
          this.queryParams.orgName = seArr[0].name
      ...
      async getData() {
        ...
        if(!this.queryParams.sectionId){
          ...
          this.queryParams.roadName = value.roadName;
            ...
            var statLat = this.queryParams.statLat;
            var rptUrl = "/ReportServer?reportlet=report/fitcheck/fitting"+this.rptPrex+"/";
            var rptLogType = "";
            if(statLat == "vehicleType"){
                rptUrl = rptUrl + "rpt_fitting_001_01_拟合按收费单元分车型通行费报表.cpt";
                rptLogType = "rpt_fitting_001_01";
            }else if(statLat == "vehicleClass"){
                rptUrl = rptUrl + "rpt_fitting_001_02_拟合按收费单元分车种通行费报表.cpt";
                rptLogType = "rpt_fitting_001_02";
            }
      ...
      watchAttrFn(newVal,oldVal,selectData){
        let selectDataShow = []
        const sectionId = newVal.sectionId
        const tollInterType = newVal.tollInterType
        if(tollInterType) {
          selectData.forEach(x => {
              if ((',' + sectionId + ',').indexOf(',' + x.roadId + ',') > -1 && tollInterType === x.gantryType) {
          ...
          selectData.forEach(x => {
              if ((',' + sectionId + ',').indexOf(',' + x.roadId + ',') > -1) {
                selectDataShow.push(x)
              }
            }
          )
