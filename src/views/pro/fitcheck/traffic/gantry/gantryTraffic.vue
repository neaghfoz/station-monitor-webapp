<template>
  <div class="app-container">
    ...
    >
      <el-row>
        <el-form-item label="途径路段" prop="sysOrgIdStr">
<!--          <ti-sys-org
            ref="sysOrg"
      ...
      <el-row>
        <el-form-item label="收费门架" size="mini">
          <ti-select
            ...
            :watch-attr="appForm.sysOrgIdStr"
...
import tableOption from "@/views/pro/fitcheck/traffic/gantry/tableOption"
import { listByReqParam } from "@/views/pro/fitcheck/traffic/gantry/api/auditFittingVcvtRptApi"
import moment from "moment"
import chartOption from "@/views/pro/fitcheck/traffic/gantry/chartOption"
import tiSysOrg from "@/views/pro/common/tiElement/tiSysOrg/tiSysOrg"

export default {
  name: 'fitcheck.traffic.gantry.gantryTrafficView',
  mixins: [...mixin],
  ...
  computed: {
    columns () {
      return this.table.showDefault == 'vc' ? tableOption.vcColumns(this.gantryId,this.tableInfo) : tableOption.vtColumns(this.gantryId,this.tableInfo)
    }
  },
  mounted () {
    // 初始化图表
  ...
  methods: {
    // 默认路段设置
    sectionFilter (datas) {
      this.sectionIds = datas
      this.sectionIds.forEach((x) => {
        this.sectionMap[x.roadId] = x.roadName
      })
      if (!this.appForm.sysOrgIdStr && datas.length > 0) {
        this.appForm.sysOrgIdStr = datas[0].roadId
        // 获取到路段之后查询
        this.submit()
      }
      return datas
    ...
    async reload (isCache) {
      ...
      this.tableInfo.gantryName = this.gantryMap[this.gantryId]||this.gantryId
      this.tableInfo.roadName = this.sectionMap[this.appForm.sysOrgIdStr]||this.appForm.sysOrgIdStr
      this.tableInfo.beginDay = this.appForm.startDay
      this.tableInfo.endDay = this.appForm.endDay
