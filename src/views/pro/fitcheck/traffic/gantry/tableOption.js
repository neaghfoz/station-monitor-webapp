...
export default {
  // 统计数据
  totalSum: (groupField, item, hjsum, field, oriField, lstField, isSum) => {
  ...
  dataFormat: function (dataArr, gantryId, gantryMap) {
    ...
    let datas = dataGroup(dataArr, null, function (item) {
      ...
      for (let index = 0; index < vehicleClassArr.length; index++) {
        ...
        let lstField = 'vcsum'
        $totalSum(groupByField, item, hjsum, field, oriField, lstField, true)
      }
      ...
      for (let index = 0; index < vehicleTypeArr.length; index++) {
        ...
        if (hasType) {
          $totalSum(groupByField, item, hjsum, field, oriField, lstField, false)
        }
      }
