...
export default {
  //统计一天的数据，按补点和完整路劲展示
  daySum(item, hjsum, field, totalField, lstField) {
    const addField = "addV" + totalField.substring(1)
    // 完整车流量
    hjsum[item.pathType + "_1"] = hjsum[item.pathType + "_1"] || {}
    hjsum[item.pathType + "_1"][field] = hjsum[item.pathType + "_1"][field] || 0
    hjsum[item.pathType + "_1"][field] += item[totalField] - item[addField]
    // 最后一列合计
    hjsum[item.pathType + "_1"][lstField] = hjsum[item.pathType + "_1"][lstField] || 0
    hjsum[item.pathType + "_1"][lstField] += item[totalField] - item[addField]

    // 补点车流量
    hjsum[item.pathType + "_2"] = hjsum[item.pathType + "_2"] || {}
    hjsum[item.pathType + "_2"][field] = hjsum[item.pathType + "_2"][field] || 0
    hjsum[item.pathType + "_2"][field] += item[addField]
    // 最后一列合计
    hjsum[item.pathType + "_2"][lstField] = hjsum[item.pathType + "_2"][lstField] || 0
    hjsum[item.pathType + "_2"][lstField] += item[addField]

    // 合计车流量
    hjsum[item.pathType + "_3"] = hjsum[item.pathType + "_3"] || {}
    hjsum[item.pathType + "_3"][field] = hjsum[item.pathType + "_3"][field] || 0
    hjsum[item.pathType + "_3"][field] += item[totalField]
    // 最后一列合计
    hjsum[item.pathType + "_3"][lstField] = hjsum[item.pathType + "_3"][lstField] || 0
    hjsum[item.pathType + "_3"][lstField] += item[totalField]

    // 总合计
    hjsum["5_1"] = hjsum["5_1"] || {}
    hjsum["5_1"][field] = hjsum["5_1"][field] || 0
    hjsum["5_1"][field] += item[totalField]
    // 总合计最后一列合计
    hjsum["5_1"][lstField] = hjsum["5_1"][lstField] || 0
    hjsum["5_1"][lstField] += item[totalField]
  },
  vcColumns: (transDay,tableInfo) => {
    return [{
        title: () => { return tableInfo.roadName + '[' + (tableInfo.beginDay==tableInfo.endDay?tableInfo.beginDay:(tableInfo.beginDay+':'+tableInfo.endDay))+']' },
        align: 'center',
        children: [{
            title: "通行日期",
            align: 'center',
    ...
    if (!transDay) {
      let datas = dataGroup(dataArr, ["transDay", "pathType"], function (item) {
        ...
        for (let index = 0; index < vehicleClassArr.length; index++) {
          ...
          sum[columnName] = item[columnName] || 0

          // 天合计
          hjsum[item.transDay] = hjsum[item.transDay] || {}
          hjsum[item.transDay][columnName] = hjsum[item.transDay][columnName] || 0
          hjsum[item.transDay][columnName] += item[columnName]
          // 总合计
          hjsum[allField] = hjsum[allField] || {}
          hjsum[allField][columnName] = hjsum[allField][columnName] || 0
          hjsum[allField][columnName] += item[columnName]

          // 最后一列合计
          field = 'vcsum';
          sum[field] = sum[field] || 0;
          sum[field] += item[columnName]
          // 天合计
          hjsum[item.transDay] = hjsum[item.transDay] || {}
          hjsum[item.transDay][field] = hjsum[item.transDay][field] || 0
          hjsum[item.transDay][field] += item[columnName]
          // 总合计
          hjsum[allField] = hjsum[allField] || {}
          hjsum[allField][field] = hjsum[allField][field] || 0
          hjsum[allField][field] += item[columnName]
        }
        ...
        for (let index = 0; index < vehicleTypeArr.length; index++) {
          ...
          sum[columnName] = item[columnName]
