    ...
    data() {
      return {
        ...
        columns: [
          ...
          {title: '门架编号', field: 'gantryId', minWidth: 170, showOverflow: true},
          {title: '门架名称', field: 'gantryName', minWidth: 170, showOverflow: true},
          /*{title: '是否虚拟门架', field: 'virGantryFlagText', minWidth: 80, showOverflow: true},
          {title: 'ip', field: 'serverIp', minWidth: 150, showOverflow: true},*/
          {title: '服务器', field: 'serverName', minWidth: 100, showOverflow: true},
          {title: '是否启用', field: 'monitorUse', minWidth: 100, showOverflow: true, slots: {default: 'isUse_operation'}},
          /*{title: '操作', width: 150, fixed: 'right', align: 'center', slots: {default: 'operation'}},*/
        ]
      }
    },
    created() {
      // if (this.queryParams.model == 'gantry') {
      //   this.getData();
      // }
    },
    methods: {
      ...
      async update(row) {
        try {
          this.loading = true
          await update({}, {model: this.queryParams.model, useId: row.useId, isUse: row.monitorUse,moduleType:'monitor'})
      ...
      async updateBatch(flag) {
        try {
          ...
          await updateBatch({}, {model: this.queryParams.model, useIdList: useIdList, isUse: flag,moduleType:'monitor'});
