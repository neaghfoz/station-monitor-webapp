<template>
  <div class="app-container">
    ...
    <el-row v-show="showConfig.lane=='true'">
      ...
      >
        <template v-slot:toolbar_buttons>
          <span style="font-weight: bold;font-size: 18px">车道——连通性监视 <span
            :class="laneData.normalData==0?'green':'red'">{{laneData.normalData}}</span>/{{laneData.page.total}}</span>
    ...
    <el-row v-show="showConfig.road=='true'">
      ...
      >
        <template v-slot:toolbar_buttons>
          <span style="font-weight: bold;font-size: 18px">路段——连通性监视 <span
            :class="roadData.normalData==0?'green':'red'">{{roadData.normalData}}</span>/{{roadData.page.total}}</span>
...
<script>
  ...
  import {findPage} from './heartBeatApi'
  import dictUtils from 'ecip-web/utils/dictUtils'

  export default {
    name: 'monitor.datamonitor.heartBeat.heartBeatView',
    components: {},
    mixins: [...mixin],
    data() {
      return {
        laneData: {
          ...
          tableToolbar: {
            id: 'monitor.datamonitor.heartBeat.heartBeatView-toolbar',
            custom: true,
            slots: {
