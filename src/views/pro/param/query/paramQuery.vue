<template>
  <div class="app-container">
    <el-row :gutter="24">
      ...
      <el-col id="paramInfoId" :span="18">
        ...
        <el-form ref="searchForm" :model="queryParams" inline :label-width="'100px'">
          <el-form-item
            v-for="(domain,index) in nodeData.qryColumns"
            :key="domain.columnName"
            :label="domain.label"
          >
            <el-input v-if="nodeData.queryList[index].modes=='input'" v-model="queryParams[domain.columnName]" />
            <ti-select v-if="nodeData.queryList[index].modes=='select'" v-model="queryParams[domain.columnName]"  clearable :dict-type="nodeData.queryList[index].dictType" :multiple="nodeData.queryList[index].multiple=='true'" style="width: 140px" />
            <el-date-picker v-if="nodeData.queryList[index].modes=='timeRange'"
              v-model="queryParams[domain.columnName]"
              type="datetimerange"
            ...
            <ti-input-range v-if="nodeData.queryList[index].modes=='regularRange'"
                            v-model="queryParams[domain.columnName]"></ti-input-range>
          </el-form-item>
        ...
        <el-card shadow="never"    class="el-card-define">
          ...
          <div slot="header" class="clearfix">
            ...
            <span v-if="selectedNode.tableName" style="color:red;float: right;padding-right: 10px">*注意：需要手动点击查询按钮进行查询</span>
          </div>
          <div class="box">
            <vxe-grid
              ref="xTable"
            ...
            >
              <template v-slot:toolbar_buttons>
                ...
                <el-button v-permission="['param:list']" type="primary" @click="exportExcel.dialog = true">导出</el-button>
              </template>
    ...
    <!--导出-->
    <ti-export-excel :exportExcel="exportExcel" :page="nodeData.page"></ti-export-excel>
...
import TiInputRange from "@/views/pro/common/tiElement/tiInput/tiInputRange";
export default {
  name: 'ParamQueryView',
  mixins: [...mixin],
  components: {TiInputRange, TiImport,TiExportExcel,TiSelect,TiDateRange},
  data() {
    return {
      importVisible: false,
  ...
  methods: {
    ...
    async getData() {
      if (this.tempParams.id) {
        ...
        let queryParam = this.paramData[this.selectedNode.tableName];
        let errFlag = false;
        queryParam.queryList.forEach((item, index, arr) => {
          if(item['modes']=='timeRange' && item['value']){
            item['value'] = item['value'][0] + ',' + item['value'][1];
          }
          if(item['modes']=='regularRange' && item['value']){
            if( (item['value'].start=='' && item['value'].end!='')|| (item['value'].start!='' && item['value'].end=='')){
              this.$notify.error('请输入完整的范围')
              errFlag = true
              return
            }
        ...
        paramList(Object.assign({}, { size: this.nodeData.page.pageSize, current: this.nodeData.page.currentPage }), queryParam).then(res => {
        ...
        //保存导出的查询条件
        queryParam.title = this.selectedNode.title;
        this.exportExcel.queryParams = queryParam;
    ...
    async paramTableClick(node, Node) {
      ...
      if (Node.level && Node.level !== 1) {
        ...
        this.nodeData.qryColumns = []
        this.nodeData.queryList = this.paramData[node.tableName].queryList;
