...
<script>
  ...
  export default {
    name: 'report.fitting.rpt_fitting_001',
      ...
      async getData() {
        ...
        if(!this.queryParams.sectionId){
          ...
          this.queryParams.roadName = value.roadName;
            ...
            var rptUrl = "/ReportServer?reportlet=report/fitting/";
            var rptLogType = "";
            if(statLat == "vehicleType"){
                rptUrl = rptUrl + "rpt_fitting_001_01_拟合按收费单元分车型通行费报表.cpt";
                rptLogType = "rpt_fitting_001_01";
                this.queryParams.__filename__ = this.queryParams.roadName+"-"+"拟合按收费单元分车型通行费报表-" +moment(new Date()).format('YYYYMMDD');
            }else if(statLat == "vehicleClass"){
                rptUrl = rptUrl + "rpt_fitting_001_02_拟合按收费单元分车种通行费报表.cpt";
                rptLogType = "rpt_fitting_001_02";
                this.queryParams.__filename__ = this.queryParams.roadName+"-"+"拟合按收费单元分车种通行费报表-" +moment(new Date()).format('YYYYMMDD');
            }
