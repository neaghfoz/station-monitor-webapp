...
<script>
  ...
  export default {
    name: 'report.fitting.rpt_fitting_road_01',
      ...
      async getData() {
        ...
        if(!this.queryParams.sectionId){
          ...
          this.queryParams.roadName = value.roadName;
            ...
            this.queryParams.__filename__ = this.queryParams.roadName+"-"+"拟合应收车流量按路段维度报表-" +moment(new Date()).format('YYYYMMDD');
            this.reportUrl = fineReportUtil.getUrl('/ReportServer?reportlet=report/fitting/rpt_fitting_road_01_拟合应收车流量按路段维度报表.cpt',this.queryParams);
