...
<script>
  ...
  export default {
    name: 'report.theory.rpt_theory_road_02',
      ...
      async getData() {
        ...
        if(!this.queryParams.sectionId){
          ...
          this.queryParams.roadName = value.roadName;
            ...
            this.queryParams.__filename__ = this.queryParams.roadName+"-"+"理论应收通行费按路段维度报表-" +moment(new Date()).format('YYYYMMDD');
            this.reportUrl = fineReportUtil.getUrl('/ReportServer?reportlet=report/theory/rpt_theory_road_02_理论应收通行费按路段维度报表.cpt',this.queryParams);

            //更新查询统计次数
            updateRptSearchTimes('rpt_theory_road_02');
