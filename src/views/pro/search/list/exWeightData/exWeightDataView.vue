...
<!--                           end-key="endDate"/>            -->
          ...
          <el-form-item label="轴数" prop="alexCount">
            <el-input v-model="queryParams.alexCount" maxlength="5" style="width: 199px" oninput="value=value.replace(/[^\d]/g, '')"/>
          </el-form-item>
    ...
    >
      <template v-slot:toolbar_buttons>
        <el-button v-permission="['exWeightData:exportExcel']" @click="exportExcel.dialog = true">导出</el-button>
      </template>

      <template v-slot:operation="{ row }">
        <el-button v-permission="['exWeightData:detail']" type="warning" @click="detail(row.hgSeq, row.enTolllaneId, row.checkTimeText)">详情</el-button>
      </template>
    </vxe-grid>
...
<script>
  ...
  import {findPage, exportExcel} from "./exWeightDataApi";
  ...
  export default {
    name: 'search.list.exWeightData.exWeightDataView',
    ...
    data() {
      return {
        ...
        importUrl: 'api/v1/search/list/exWeightData/importExcel',
        tplUrl: 'api/v1/search/list/exWeightData/excelTemplate',
        exportUrl: 'api/v1/search/listexWeightData/exportExcel?token=${getToken()}',
        tableToolbar: {
          id: 'search.list.exWeightData.exWeightDataView-toolbar',
          custom: true,
          slots: {
    ...
    methods: {
      ...
      detail(hgSeq, enTolllaneId, checkTime) {
        console.log("hgSeq" + hgSeq +":enTolllaneId" + enTolllaneId +"checkTime:" + checkTime );
        var timestamp = Date.parse(new Date());
        this.$router.push({
          path : '/search/list/exWeightData/detail/' + timestamp,
          query : {
            hgSeq : hgSeq,
