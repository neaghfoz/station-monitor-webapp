...
<script>
  ...
  import { findUseLanes, getData, addUpdation } from '../config'
  export default {
    name: 'AutoUpdationConfigIssueViewNotCached',
    components: { },
    mixins: [...mixin],
    data() {
      return {
    ...
    methods: {
      init() {
        ...
        if (this.inOutType) {
          this.inOutType.forEach((item, index) => {
            ...
            this.inOutTypeMap['inOutType' + item.value] = item.label
          })
        }
        ...
        if (checked) {
          this.useLanes.forEach((item, index) => {
            if (value && value.indexOf('laneType') >= 0 && value === ('laneType' + item.laneType)) {
              if(this.checkedLanes.indexOf(item.laneId)<0){
                this.checkedLanes.push(item.laneId)
              }
            }
      ...
      save() {
        var data = { versionId: this.queryParams.versionId, terminals: [] }
        ...
        this.checkedLanes.forEach((item, index) => {
          data.terminals.push(laneMap[item] )
        })
        this.saveLoading = true
        addUpdation(data).then((res) => {
