...
<script>
  ...
  export default {
    ...
    components: {TiExportExcel, failList},
    mixins: [...mixin],
    data() {
      return {
        exportExcel: {
                ...
                title: '站省发送失败数', field: 'spSendFailCnt', minWidth: 80, showOverflow: true, align: 'right', slots: {
                  default: ({row}, h) => {
                    if (row.spSendFailCnt > 0) {
                      if(this.showConfig.showFailReaSon) {
                        return [<a  style= "color: white;background: red;" onClick={() => this.details(row, 2)}>{row.spSendFailCnt}</a>]
                      } else {
                ...
                title: '站部发送失败数', field: 'sdSendFailCnt', minWidth: 80, showOverflow: true, align: 'right', slots: {
                  default: ({row}, h) => {
                    if (row.sdSendFailCnt > 0) {
                      if(this.showConfig.showFailReaSon) {
                        return [<a  style= "color: white;background: red;" onClick={() => this.details(row, 3)}>{row.sdSendFailCnt}</a>]
                      } else {
    ...
    //   'queryParams.sysOrgIdStr_lane'(newVal,oldVal) {
    ...
    computed: {
      showConfig() {
        return {
          showFailReaSon: (dictUtils.getDictLabel("tibms_config", "mo_server_transSta_showFailReaSon") === 'true')
        }
      }
    },
    methods: {
      ...
      details(row, linkType) {
        ...
        this.$refs.failListDialog.init(title, row, linkType)
      }
    }
  }
