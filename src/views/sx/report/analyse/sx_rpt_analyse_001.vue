    ...
    <el-card>
      <el-form ref="searchForm" :model="queryParams" inline :label-width="'100px'" size="small">
        ...
        <el-form-item label="统计类型" prop="dateStatisticType">
          <ti-select :clearable="false" v-model="queryParams.dateStatisticType" dict-type="tibms_rpt_collectdatetype"/>
        </el-form-item>
...
<script>
  ...
  export default {
    name: 'report.analyse.sx_rpt_analyse_001',
      ...
      async getData() {
        ...
        if (this.queryParams.dateStatisticType === 'day') {
          ...
          this.reportUrl = fineReportUtil.getUrl(
            '/ReportServer?reportlet=report/analyse/sx_rpt_analyse_001_01_出口日旬报表.cpt&__filename__=出口日旬报表'+XEUtils.toDateString(new Date(), 'yyyyMMddmmddss'),
            this.queryParams);
