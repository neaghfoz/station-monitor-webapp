  ...
  <div class="app-container" style="height: 100%">
    <el-card>
      <el-form ref="searchForm" :model="queryParams" inline :label-width="'100px'" size="small">
        ...
        <el-form-item label="统计类型" prop="payType">
          <ti-select :clearable="false" v-model="queryParams.payType" dict-type="tibms_rpt_paytype"/>
        </el-form-item>
        <el-form-item label="出入口类型" prop="inOutType" label-width="'50px'">
          <ti-select :clearable="false" v-model="queryParams.inOutType" dict-type="tibms_com_inOutType"/>
        </el-form-item>
...
<script>
  ...
  export default {
    name: 'report.common.sx_rpt_common_001',
    ...
    methods: {
      async getData() {
        ...
        this.reportUrl = fineReportUtil.getUrl(
          '/ReportServer?reportlet=report/common/sx_rpt_common_001_01_免费车辆车型汇总报表.cpt&__filename__=免费车辆车型汇总报表'+XEUtils.toDateString(new Date(), 'yyyyMMddmmddss'),
          this.queryParams);
