...
<script>
  ...
  export default {
    name: 'report.common.sx_rpt_common_010',
    ...
    data() {
      return {
        ...
        queryParams: {
          ...
          startDay:moment(new Date()).startOf("month").format("YYYY-MM-DD"),
    ...
    methods: {
      async getData() {
        ...
        this.reportUrl = fineReportUtil.getUrl(
          '/ReportServer?reportlet=report/common/sx_rpt_common_010_01_票据统计报表.cpt&__filename__=票据统计报表'+XEUtils.toDateString(new Date(), 'yyyyMMddmmddss'),
          this.queryParams);

        //更新查询统计次数
        //updateRptSearchTimes('sx_rpt_common_010_01_票据统计报表');
