...
<script>
  ...
  export default {
    name: 'report.etc.sx_rpt_etc_004',
      ...
      async getData() {
        ...
        if (this.queryParams.dateStatisticType === 'day') {
          ...
          dateTrans = dateUtil.collectTypeCombination(this.queryParams.startMonth,
            this.queryParams.startMonth, this.queryParams.collectType,
            this.queryParams.dateStatisticType);
          this.queryParams.collectDateStr = '日期：'+ this.queryParams.startMonth;
        ...
        if (this.queryParams.shiftID == null || this.queryParams.shiftID == ''){
          this.queryParams.shiftNo = '全部'
        } else{
        ...
        this.reportUrl = fineReportUtil.getUrl(
          '/ReportServer?reportlet=report/etc/sx_rpt_ETC_004_01_新ETC出口交通流量报表.cpt&__filename__=新ETC出口交通流量报表'+XEUtils.toDateString(new Date(), 'yyyyMMddmmddss'),
          this.queryParams);
